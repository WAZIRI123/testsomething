C:\wamp64\www\akaunting\resources\views\purchases\bills\show.blade.php
...
C:\wamp64\www\akaunting\app\Http\Middleware\PortalMenu.php?
...
$editRoute=''
$parameter=1
$this->editRoute = $this->geteditRoute($type, $editRoute)=$this->geteditRoute(bill, '');

$route = $this->getRouteFromConfig($type, $config_key, $config_parameters = [], $modal = false)=$this->getRouteFromConfig(bill, 'edit', 1, true);
...
OPTION1
$route = config('type.' . static::OBJECT_TYPE . bill . '.route.' . $config_key)=config('type . document .bill . route . edit)=undefined
...
$route .= edit;
if ($modal=true)
route(modals.bills.edit, 1);

OPTION3
$route = Str::plural(bill, 2) . '.' . edit;
route(bills.edit,1);
...
FINAL
$route=modals.bills.edit
or
$route=bills.edit this it final
...
$alias = config('type.' . static::OBJECT_TYPE . '.' . $type . '.alias')=config('type.document.bill.alias')='';

if (!empty($alias)) {
            $route .= $alias . '.';
        }

$route .= 'settings.bill.edit';
...
$editRoute=''
$parameter=1
$this->editRoute = $this->getDeleteRoute($type, $editRoute)=$this->geteditRoute(bill, '');

$route = $this->getRouteFromConfig($type, $config_key, $config_parameters = [], $modal = false)=$this->getRouteFromConfig(bill, 'destroy', 1);
...
OPTION1
$route = config('type.' . static::OBJECT_TYPE . bill . '.route.' . $config_key)=config('type . document .bill . route . destroy)=undefined
...
$route .= destroy;
if ($modal=true)
route(modals.bills.destroy, 1);

OPTION3
$route = Str::plural(bill, 2) . '.' . edit;
route(bills.destroy,1);

OPTION4
$route=''

FINAL
$route=modals.bills.destroy
or
$route=bills.destroy this it final
